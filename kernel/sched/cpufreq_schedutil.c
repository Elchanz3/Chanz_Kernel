                                   ...
                                   unsigned int flags)
{
    ...
    } else if (sg_cpu->iowait_boost && (time - sg_cpu->last_update > TICK_NSEC)) {
                               ...
                               unsigned long *max)
{
    if (sg_cpu->iowait_boost) {
        if (*util * sg_cpu->iowait_boost_max < *max * sg_cpu->iowait_boost) {
            *util = sg_cpu->iowait_boost;
            *max = sg_cpu->iowait_boost_max;
        }
...
static unsigned long freq_to_util(struct sugov_policy *sg_policy,
                                  unsigned int freq)
{
    return mult_frac(sg_policy->max, freq, sg_policy->policy->cpuinfo.max_freq);
}
                               ...
                               u64 upto)
{
    ...
    /* Track cycles in the current window */
    u64 delta_ns = (upto - sg_policy->last_cyc_update_time) * prev_freq;
    u64 cycles = delta_ns / (NSEC_PER_SEC / KHZ);
                               ...
                               unsigned int prev_freq)
{
    ...
    BUG_ON(curr_ws < sg_policy->last_ws);

    if (curr_ws <= sg_policy->last_ws)
        return;

    if (curr_ws > (sg_policy->last_ws + sched_ravg_window)) {
        sg_policy->avg_cap = freq_to_util(sg_policy, prev_freq);
        sg_policy->last_cyc_update_time = curr_ws;
    } else {
        sugov_track_cycles(sg_policy, prev_freq, curr_ws);
        sg_policy->avg_cap = freq_to_util(sg_policy, sg_policy->curr_cycles /
                                          (sched_ravg_window / (NSEC_PER_SEC / KHZ)));
    }
                              ...
                              unsigned long *max)
{
    ...
    bool is_hiload = (cpu_util >= mult_frac(sg_policy->avg_cap,
...
{
    bool ret = (tick_nohz_get_idle_calls() == sg_cpu->saved_idle_calls);
    sg_cpu->saved_idle_calls = tick_nohz_get_idle_calls();
    return ret;
}
#else
                                ...
                                unsigned int flags)
{
    ...
    if (flags & SCHED_CPUFREQ_RT_DL) {
        ...
        sugov_calc_avg_cap(sg_policy, sg_cpu->walt_load.ws, sg_policy->policy->cur);
        trace_sugov_util_update(sg_cpu->cpu, sg_cpu->util, sg_policy->avg_cap,
                                max, sg_cpu->walt_load.nl, sg_cpu->walt_load.pl, flags);
        sugov_iowait_boost(sg_cpu, &util, &max);
        sugov_walt_adjust(sg_cpu, &util, &max);
        
        next_freq = get_next_freq(sg_policy, util, max);
        
        // Avoid reducing the frequency if the CPU has been busy
        if (busy && next_freq < sg_policy->next_freq)
            next_freq = sg_policy->next_freq;
    }
    
    sugov_update_commit(sg_policy, time, next_freq);
...
static unsigned int sugov_next_freq_shared(struct sugov_cpu *sg_cpu, u64 time)
{
    ...
    for_each_cpu(j, policy->cpus) {
        ...
        // If the CPU utilization is stale, clear iowait_boost and skip
        ...
        // Update util and max based on the current CPU's values
                                ...
                                unsigned int flags)
{
    ...
    if (sugov_should_update_freq(sg_policy, time)) {
        if (flags & SCHED_CPUFREQ_RT_DL) {
            ...
            next_freq = sugov_next_freq_shared(sg_cpu, time);
        }
        sugov_update_commit(sg_policy, time, next_freq);
    }
...
static void sugov_work(struct kthread_work *work)
{
    ...
    sugov_track_cycles(sg_policy, sg_policy->policy->cur, sched_ktime_clock());
